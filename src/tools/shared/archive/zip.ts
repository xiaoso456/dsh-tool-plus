...
/**
 * DSH port note: OMP imported `formatBytes` from `@oh-my-pi/pi-utils`, which we
 * do not depend on. Inlined verbatim from packages/utils/src/format.ts.
 */
...
/**
 * DSH port note: this runtime is Node, not Bun. Archive *reads* load the whole
 * container into memory (bounded below); archive *writes* frame ZIP through the
 * in-process `zip()` framer and tar/tar.gz through an in-process ustar writer
 * (replacing OMP's `Bun.Archive`). Read laziness for ZIP was traded for the
 * bounded full-buffer path — semantics are identical, huge containers reject.
 */
const ARCHIVE_FULL_READ_LIMIT = 512 * 1024 * 1024;

async function readFileBytesLimited(filePath: string): Promise<Uint8Array> {
	const st = await fsp.stat(filePath);
	if (!Number.isSafeInteger(st.size) || st.size > ARCHIVE_FULL_READ_LIMIT) {
...
async function fileByteSource(filePath: string): Promise<ByteSource> {
	// DSH port note: OMP lazily ranged-read the ZIP here via `Bun.file`; on Node
	// we hand the framer an in-memory source (bounded by ARCHIVE_FULL_READ_LIMIT).
	return memoryByteSource(await readFileBytesLimited(filePath));
}
...
export async function openArchive(source: ArchiveSource): Promise<ArchiveReader> {
	...
	if (format === "zip") {
		return new ArchiveReader(format, await readZipEntries(await fileByteSource(filePath)));
	}

	const bytes = await readFileBytesLimited(filePath);
	assertTarArchiveSize(bytes.byteLength);
...
async function resolveArchiveBytes(source: ArchiveSource): Promise<{ bytes: Uint8Array; format: ArchiveFormat }> {
	...
	const bytes = await readFileBytesLimited(filePath);
	if (format !== "zip") assertTarArchiveSize(bytes.byteLength);
	return { bytes, format };
}
 ...
 * DSH port note: ZIP is framed in memory by `zip()`; tar / tar.gz are framed in
 * process by the ustar writer below (replacing OMP's `Bun.Archive`; gzip via
 * `node:zlib`). String members are encoded as UTF-8.
 */
export async function writeArchive(
	...
	if (format === "zip") {
		...
		await fsp.writeFile(destPath, zip(record));
		return;
	}
	...
	const tarBytes = writeUstar(record);
	await fsp.writeFile(destPath, format === "tar.gz" ? zlib.gzipSync(tarBytes) : tarBytes);
}

/** Octal field writer for a ustar header: zero-padded, NUL-terminated. */
function writeUstarOctal(header: Uint8Array, offset: number, length: number, value: number): void {
	const text = value.toString(8);
	const padded = text.padStart(length - 1, "0");
	for (let index = 0; index < length - 1; index++) header[offset + index] = padded.charCodeAt(index) ?? 0x30;
	header[offset + length - 1] = 0;
}

/**
 * Frame a `path → bytes` map as a POSIX ustar tar image (512-byte headers,
 * data padded to 512-byte blocks, two zero-block terminator). Names longer
 * than 100 bytes split into a 155-byte prefix plus name when possible;
 * anything that still does not fit throws instead of corrupting the archive.
 */
function writeUstar(record: Record<string, Uint8Array>): Uint8Array {
	const blocks: Uint8Array[] = [];
	for (const name in record) {
		const data = record[name]!;
		const nameBytes = ENCODER.encode(name);
		let prefixBytes: Uint8Array | undefined;
		let headerName = nameBytes;
		if (nameBytes.byteLength > 100) {
			const slashIndex = name.slice(0, 155).lastIndexOf("/");
			if (slashIndex <= 0 || slashIndex + 1 + (name.length - slashIndex - 1) > 100 || name.length - slashIndex - 1 > 100) {
				throw new ToolError("Archive member name is too long to store in a tar archive");
			}
			prefixBytes = ENCODER.encode(name.slice(0, slashIndex));
			headerName = ENCODER.encode(name.slice(slashIndex + 1));
		}

		const header = new Uint8Array(512);
		header.set(headerName.subarray(0, 100), 0);
		writeUstarOctal(header, 100, 8, 0o644);
		writeUstarOctal(header, 108, 8, 0);
		writeUstarOctal(header, 116, 8, 0);
		writeUstarOctal(header, 124, 12, data.byteLength);
		writeUstarOctal(header, 136, 12, 0);
		header[156] = 0x30; // typeflag '0' — regular file
		header.set(ENCODER.encode("ustar"), 257);
		header[263] = 0x30; // version '0'
		header[264] = 0x30;
		// Checksum: field itself counted as 8 spaces while summing, then written
		// as 6 octal digits + NUL + space.
		let sum = 0;
		for (let index = 0; index < 512; index++) sum += index >= 148 && index < 156 ? 0x20 : header[index];
		const chksum = `${sum.toString(8).padStart(6, "0")}\u0000 `;
		for (let index = 0; index < 8; index++) header[148 + index] = chksum.charCodeAt(index) ?? 0;
		if (prefixBytes) header.set(prefixBytes.subarray(0, 155), 345);
	...
	const total = blocks.reduce((sum2, block) => sum2 + block.byteLength, 0);
...
export async function extractArchive(source: ArchiveSource, destDir: string): Promise<number> {
	...
	for (const [name, content] of entries) {
		...
		await fsp.mkdir(path.dirname(outputPath), { recursive: true }).catch(() => {});
		await fsp.writeFile(outputPath, await memberToBytes(content));
		count++;
	}
	return count;
}
