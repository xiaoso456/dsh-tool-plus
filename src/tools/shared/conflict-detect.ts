...
// Minimal ToolSession compat — only conflictHistory needed
interface ToolSession { conflictHistory?: any }
...
export async function scanFileForConflicts(
	...
	if (!st) return { blocks: [], scanTruncated: false }
	const size = st.size
	const truncated = size > maxBytes
	let text: string
	if (truncated) {
		const fd = await (await import('node:fs/promises')).open(absolutePath, 'r')
		try {
		...
		text = await readFile(absolutePath, 'utf-8').catch(() => '')
	}
