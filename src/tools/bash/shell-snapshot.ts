...
// Read the shell helper script at module load.
// In dev the file sits next to this source file; in the bundled lib/ layout
// copy-assets.mjs places it next to the bundled runtime entry, so
// `new URL('.', import.meta.url)` resolves in both.
function loadFnEnvHelper(): string {
	const candidates = [
		path.join(fileURLToPath(new URL(".", import.meta.url)), "shell-snapshot-fn-env.sh"),
		path.join(path.dirname(process.execPath), "shell-snapshot-fn-env.sh"),
	];
	...
	throw new Error(`shell-snapshot-fn-env.sh not found. Tried:\n${candidates.join("\n")}`);
}
const fnEnvHelper = loadFnEnvHelper();
...
function scrubSnapshotInPlace(snapshotPath: string): void {
	try {
		...
		runtimeLogger().debug("shell-snapshot: dropped brush-incompatible aliases", { dropped });
	} catch (err) {
		runtimeLogger().debug("shell-snapshot: scrub failed", { err: String(err) });
	}
}
...
export async function getOrCreateSnapshot(
	...
	// Windows: no .bashrc in standard location, so there is no user env to
	// capture — but still create a minimal snapshot file so plugin injections
	// (rmSafe's rm → trash redefinition) have a source point on Windows Git
	// bash. The file holds only a comment header; executeBash appends the
	// injection when rmSafe is enabled.
	if (process.platform === "win32") {
		const uid = process.getuid?.();
		const snapshotDir = path.join(
			os.tmpdir(),
			uid === undefined ? "omp-shell-snapshots" : `omp-shell-snapshots-${uid}`,
		);
		const snapshotPath = path.join(snapshotDir, `snapshot-bash-${crypto.randomUUID()}.sh`);
		try {
			fs.mkdirSync(snapshotDir, { recursive: true, mode: 0o700 });
			fs.writeFileSync(
				snapshotPath,
				"# Shell snapshot - generated by omp agent (windows minimal)\n",
				{ mode: 0o600 },
			);
			cachedSnapshotPaths.set(cacheKey, snapshotPath);
			return snapshotPath;
		} catch (err) {
			runtimeLogger().debug("shell-snapshot: windows snapshot dir unusable", {
				dir: snapshotDir,
				err: String(err),
			});
			return null;
...
/**
 * Delete every cached snapshot file and clear the cache. Called from the
 * plugin's disposal path (the pi-gateway original ran this through its
 * postmortem registry).
 */
