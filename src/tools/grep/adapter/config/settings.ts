...
/** Raw settings document shape (verbatim OMP). */
...
export class Settings {
  ...
  /** OMP storage accessor (DSH: no agent storage — null). */
  getStorage(): null {
    return null
  }

  /** Whether a path is explicitly configured (DSH config is always resolved). */
  isConfigured(_path: string): boolean {
    return true
  }

  /** Persist a setting (DSH: config is static at tool time — no-op). */
...
/** Re-exported schema default lookup (OMP settings.ts re-exports it too). */
export { getDefault } from './settings-schema.ts'
