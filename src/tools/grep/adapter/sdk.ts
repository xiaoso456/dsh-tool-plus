/**
 * DSH adapter for OMP `sdk.ts` — the `ToolSession` interface the grep
 * engine consumes.
 *
 * Only the members the grep engine (and its omp dependency closure) actually
 * touches are declared (verbatim shapes where applicable); everything else in
 * OMP's session (tools registry, settings storage, LSP, MCP, IRC, …) is
 * DSH-side and intentionally absent. Derived from the read adapter's sdk (see
 * `src/tools/read/adapter/sdk.ts`); differs by dropping read-only types
 * (xdev/conflict-history) and adding `getSessionSpawns`, which grep's scout
...
/**
 * Minimal ToolSession surface used by the OMP grep engine (verbatim member
...
export interface ToolSession {
  ...
  hasEditTool?: boolean
  /** Session settings (grep.* keys; DSH maps them onto tool-plus config). */
  settings: Settings
  ...
  /** Session spawn policy (DSH: none — grep falls back to `"*"`). */
