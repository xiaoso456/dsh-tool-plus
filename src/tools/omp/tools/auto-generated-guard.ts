 ...
 *
 * 唯一实现（A-3，second-impl-audit.md）：本文件是 OMP refs
 * packages/coding-agent/src/tools/auto-generated-guard.ts 的逐字移植，此前
 * 曾与 `src/tools/shared/auto-generated-guard.dsh.ts`（简化重写）并存，现已
 * 收敛到此处；`src/tools/edit/adapter/tools/auto-generated-guard.ts` 与
 * `src/tools/shared/auto-generated-guard.dsh.ts` 仅保留纯转发。
 * 唯一偏离（T17-1 修复，随合并带入）：basename 提取兼容 Windows 反斜杠路径。
 */
import * as path from "node:path";
import { isEnoent, peekFile } from "@oh-my-pi/pi-utils";
import { LRUCache } from "@oh-my-pi/pi-utils/lru";
import { settings as globalSettings, isSettingsInitialized, type Settings } from "../config/settings.ts";
...
function isAutoGeneratedFileName(filePath: string): boolean {
	// T17-1：Windows 路径用 `\` 分隔，上游 `split("/")` 取不到 basename（整串
	// 不匹配 `/^zz_generated\./` 等模式，守卫静默放行）——同时按 `/` 和 `\` 切。
...
async function getAutoGeneratedMarker(filePath: string): Promise<string | undefined> {
	if (isAutoGeneratedFileName(filePath)) {
		// T17-1：与 isAutoGeneratedFileName 同步兼容反斜杠 basename。
