/**
 * DSH adapter for OMP `sdk.ts` — the `ToolSession` interface the write
 * engine consumes.
 *
 * Only the members the write engine actually touches are declared (verbatim
...
import type { XdevState } from './omp/xdev.ts'
import type { ConflictHistory } from './omp/conflict-detect.ts'
...
/**
 * Minimal ToolSession surface used by the OMP write engine (verbatim member
...
export interface ToolSession {
  ...
  hasEditTool?: boolean
  /** Session settings (write/lsp.* keys; DSH maps onto tool-plus config). */
  settings: Settings
  ...
  getActiveModel?: () => import('@oh-my-pi/pi-ai').Model | undefined
  ...
  /** Diagnostics ledger (no LSP in this engine — inert but present for engine types). */
