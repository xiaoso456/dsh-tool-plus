/**
 * DSH adapter for OMP `sdk.ts` — the `ToolSession` interface the read
 * engine consumes.
 *
 * Only the members the read engine actually touches are declared (verbatim
...
/**
 * Minimal ToolSession surface used by the OMP read engine (verbatim member
