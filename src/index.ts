...
import { registerRead } from './tools/read/index.ts'
import { registerWrite } from './tools/write/index.ts'
import { registerEdit } from './tools/edit/index.ts'
import { registerGrep } from './tools/grep/index.ts'
import { registerGlob } from './tools/glob/index.ts'
import { registerAstEdit } from './tools/ast-edit/index.ts'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  installBashPlusSettings(ctx, config, (current) => { cfg = current(); setBlockAutoGenerated(cfg.editBlockAutoGenerated) })
  ...
  registerRead(ctx, () => ({
    enabled: cfg.readSummarizeEnabled,
    minBodyLines: cfg.readSummarizeMinBodyLines,
    minCommentLines: cfg.readSummarizeMinCommentLines,
    minTotalLines: cfg.readSummarizeMinTotalLines,
    unfoldUntilLines: cfg.readSummarizeUnfoldUntil,
    unfoldLimitLines: cfg.readSummarizeUnfoldLimit,
  }))
  registerWrite(ctx)
  registerEdit(ctx)
  registerGrep(ctx)
  ...
  ctx.systemPrompt.section({
    name: 'tool:read',
    order: 100,
    text: 'Use the read tool — not shell cat/head/tail — to inspect files. `file_path` accepts an inline selector: :N (single line), :N-M (range), :N+K (K lines from N), :N- (to EOF), comma multi-range, :raw (verbatim). Archive members use "archive.zip:inner/path".',
  })
  ctx.systemPrompt.section({
    name: 'tool:write',
    order: 101,
